...
def get_git_state() -> dict:
    ...
    try:
        ...
        # Uncommitted diff
        diff = subprocess.run(
            ["git", "diff", "HEAD"], capture_output=True, text=True, cwd=PROJECT_ROOT
        ).stdout
...
def auto_detect_file_categories() -> Dict[str, List[str]]:
    ...
    for src_dir in src_dirs:
        ...
        if src_path.exists():
            for ext in [
                ...
                )  # Limit to 50 files per category
    ...
    if docs_path.exists():
        inventory["documentation"].extend(
            [str(f.relative_to(PROJECT_ROOT)) for f in docs_path.rglob("*.md")][:30]
        )
...
def save_checkpoint(name: Optional[str] = None, message: Optional[str] = None) -> str:
    ...
    print(
        f"  Project: {PROJECT_NAME} ({checkpoint_data['project'].get('detected_type', 'unknown')})"
    )
    print(
        f"  Git: {checkpoint_data['git'].get('branch', 'unknown')}@{checkpoint_data['git'].get('commit_short', 'unknown')}"
    )
    if checkpoint_data["git"].get("has_uncommitted_changes"):
        print(
            f"  Uncommitted changes: {checkpoint_data['git'].get('diff_size', 0)} bytes"
        )
...
def list_checkpoints(limit: int = 10) -> list:
    ...
    for cp in checkpoints:
        print(
            f"{cp['id']:<18} {cp.get('name', 'unnamed')[:23]:<25} "
...
def show_checkpoint(checkpoint_id: str) -> Optional[dict]:
    ...
    print(
        f"  Uncommitted Changes: {'Yes' if data['git'].get('has_uncommitted_changes') else 'No'}"
    )
...
def restore_checkpoint(checkpoint_id: str = None, latest: bool = False) -> bool:
    ...
    print(f"Type: {data['project'].get('detected_type', 'Unknown')}")
    print(
        f"Git: {data['git'].get('branch', 'unknown')}@{data['git'].get('commit_short', 'unknown')}"
    )
...
Examples:
  ...
  %(prog)s restore --latest
    ...
    restore_parser.add_argument(
        "--latest", action="store_true", help="Restore latest checkpoint"
    )
    ...
    list_parser.add_argument(
        "--limit", "-l", type=int, default=10, help="Max checkpoints to show"
    )
