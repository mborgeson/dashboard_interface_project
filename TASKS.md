...
### Epic 4.1: Schema Drift Detection [WS4] ✅ (2026-03-27)
- [x] UR-023: Store baseline fingerprints for each completed group [WS4] [S]
- [x] UR-023: Create SchemaDriftDetector with check_drift() method [WS4] [M]
- [x] UR-023: Define thresholds -- >=0.95 OK, 0.90-0.94 info, 0.80-0.89 warn, <0.80 error [WS4] [S]
- [x] UR-023: Integrate pre-extraction drift check into group_pipeline Phase 4 [WS4] [S]
- [x] UR-023: Drift alert persistence -- model, migration, CRUD, API endpoint [WS4] [M]
- [x] UR-023: Tests -- OK drift, warning drift, error drift, alert persistence [WS4] [M]

### Epic 4.2: Domain Validation Framework [WS4] ✅ (2026-03-27)
- [x] UR-022: Create domain_validators.py with DOMAIN_RULES and validate_domain_range() [WS4] [M]
- [x] UR-022: Integrate validation into bulk_insert() -- flag, don't reject [WS4] [S]
- [x] UR-022: 30+ unit tests for all domain rule boundary conditions [WS4] [M]
- [x] UR-022: API endpoint or report to surface validation warnings [WS4] [S]

### Epic 4.3: Frontend & Code Health [WS1, WS3] ✅ (2026-03-27)
- [x] UR-013: Standardize 6 frontend API client imports [WS1] [S]
- [x] UR-027: Delete deploy.yml.disabled artifact [WS1] [XS]
- [x] UR-028: Structured SharePoint health check -- actual Graph API call [WS1] [M]
- [x] UR-033: Stage history timeline component in Deal Detail Modal [WS3] [M]
- [x] UR-034: Manual stage override with optional reason field [WS3] [S]
- [x] UR-035: Stage mapping API endpoint [WS3] [XS]
- [x] UR-036: Handle non-canonical stages via alias mapping [WS3] [XS]

### Epic 4.4: Remaining P2 Items [WS2, WS4] ✅ (2026-03-27)
- [x] UR-029: File locking detection -- check publication.level [WS2] [S]
- [x] UR-030: ETag-based version comparison before extraction [WS2] [S]
- [x] UR-031: Content hash population -- SHA-256 after download [WS2] [S]
- [x] UR-032: Graph API rate limiting -- semaphore + Retry-After [WS2] [S]
- [x] UR-037: Close XLSB workbooks after extraction [WS4] [S]
- [x] UR-038: Increase fingerprint row scan limit to 500 [WS4] [S]
- [x] UR-039: Batch-level sum reconciliation (NOI = Revenue - OpEx) [WS4] [M]
- [x] UR-040: Stabilize duplicate field name suffixes -- cell-address-based [WS4] [M]
- [x] UR-041: Add confidence score column to ExtractedValue [WS4] [M]
