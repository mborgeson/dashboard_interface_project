...
def upgrade() -> None:
    ...
    op.execute("""
        CREATE TYPE reportcategory AS ENUM ('executive', 'financial', 'market', 'portfolio', 'custom')
    """)
    ...
    # Create schedule_frequency enum
    op.execute("""
    ...
    op.create_table('report_templates',
        ...
        sa.Column('category', postgresql.ENUM('executive', 'financial', 'market', 'portfolio', 'custom', name='reportcategory', create_type=False), nullable=False),
        sa.Column('sections', postgresql.ARRAY(sa.String()), nullable=False),
        sa.Column('export_formats', postgresql.ARRAY(sa.String()), nullable=False),
        sa.Column('is_default', sa.Boolean(), nullable=False, default=False),
        sa.Column('created_by', sa.String(length=255), nullable=False, default='System'),
    ...
    op.create_table('queued_reports',
        sa.Column('id', sa.Integer(), autoincrement=True, nullable=False),
        sa.Column('name', sa.String(length=255), nullable=False),
        sa.Column('template_id', sa.Integer(), nullable=False),
        sa.Column('status', postgresql.ENUM('pending', 'generating', 'completed', 'failed', name='reportstatus', create_type=False), nullable=False),
        sa.Column('progress', sa.Integer(), nullable=False, default=0),
        sa.Column('format', postgresql.ENUM('pdf', 'excel', 'pptx', name='reportformat', create_type=False), nullable=False),
        ...
        sa.Column('created_at', sa.DateTime(timezone=True), nullable=False),
        sa.Column('updated_at', sa.DateTime(timezone=True), nullable=False),
        sa.ForeignKeyConstraint(['template_id'], ['report_templates.id'], name=op.f('fk_queued_reports_template_id_report_templates')),
        sa.PrimaryKeyConstraint('id', name=op.f('pk_queued_reports'))
    )
    op.create_index(op.f('ix_queued_reports_id'), 'queued_reports', ['id'], unique=False)
    op.create_index(op.f('ix_queued_reports_template_id'), 'queued_reports', ['template_id'], unique=False)
    op.create_index(op.f('ix_queued_reports_status'), 'queued_reports', ['status'], unique=False)
    op.create_index(op.f('ix_queued_reports_requested_at'), 'queued_reports', ['requested_at'], unique=False)
    ...
    op.create_table('distribution_schedules',
        ...
        sa.Column('is_deleted', sa.Boolean(), nullable=False, default=False),
        sa.Column('deleted_at', sa.DateTime(timezone=True), nullable=True),
        sa.ForeignKeyConstraint(['template_id'], ['report_templates.id'], name=op.f('fk_distribution_schedules_template_id_report_templates')),
        sa.PrimaryKeyConstraint('id', name=op.f('pk_distribution_schedules'))
    )
    op.create_index(op.f('ix_distribution_schedules_id'), 'distribution_schedules', ['id'], unique=False)
    op.create_index(op.f('ix_distribution_schedules_template_id'), 'distribution_schedules', ['template_id'], unique=False)
    op.create_index(op.f('ix_distribution_schedules_is_active'), 'distribution_schedules', ['is_active'], unique=False)
    op.create_index(op.f('ix_distribution_schedules_next_scheduled'), 'distribution_schedules', ['next_scheduled'], unique=False)


def downgrade() -> None:
    """Downgrade database schema - remove reporting tables."""
    # Drop distribution_schedules
    op.drop_index(op.f('ix_distribution_schedules_next_scheduled'), table_name='distribution_schedules')
    op.drop_index(op.f('ix_distribution_schedules_is_active'), table_name='distribution_schedules')
    op.drop_index(op.f('ix_distribution_schedules_template_id'), table_name='distribution_schedules')
    op.drop_index(op.f('ix_distribution_schedules_id'), table_name='distribution_schedules')
