"""
Tempe, AZ Building Permits — ArcGIS REST API client.

Fetches multifamily building permit data from Tempe's
ArcGIS-based building, licensing, development, and safety system.
...
DEFAULT_TEMPE_LAYER_URL = (
    "https://services1.arcgis.com/Tempe/arcgis/rest/services/"
    "Building_Permits/FeatureServer/0/query"
)


async def fetch_tempe_permits(
    layer_url: str | None = None,
    months_back: int = 36,
    result_record_count: int = 2000,
) -> dict:
    """Fetch building permit data from Tempe's ArcGIS REST endpoint.
    ...
    try:
        ...
        if "error" in data:
            errors.append(
                f"Tempe ArcGIS error: {data['error'].get('message', 'Unknown')}"
            )
            return {
                "records": [],
                "errors": errors,
        ...
        for feature in features:
            ...
            # IssuedDate is in epoch milliseconds
            issued_ms = attrs.get("IssuedDate")
            if not issued_ms:
                continue
            ...
            records.append(
                {
                    "source": "tempe_blds",
                    "series_id": f"TEMPE-{permit_number}"
                    if permit_number
                    else "TEMPE-UNKNOWN",
                    "geography": "Tempe, AZ",
        ...
        errors.append(f"Tempe ArcGIS HTTP error: {e.response.status_code}")
        return {
            "records": [],
            "errors": errors,
        ...
        errors.append(f"Tempe ArcGIS request error: {e}")
        return {"records": [], "errors": errors, "api_response_code": None}

    logger.info(
        "tempe_blds_fetch_complete",
...
def save_tempe_records(
    db_session,
    records: list[dict],
    api_response_code: int | None = None,
    errors: list[str] | None = None,
) -> tuple[int, int]:
    """Save fetched Tempe permit records to the database.
